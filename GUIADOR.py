...
st.markdown(
    ...
    .preview-box {background:white; padding:18px; border-radius:8px; border:1px solid #ddd; max-width:700px; margin:auto;}
    .preview-title {text-align:center; font-size:22px; font-weight:bold; margin-bottom:0;}
    .preview-sub {text-align:center; font-size:12px; margin-top:2px; margin-bottom:12px;}
    </style>
    """,
...
if "last_data" not in st.session_state:
    st.session_state.last_data = None  # datos usados en el último PDF
...
# -----------------------
# FASE 2 - Formulario dinámico y Generar PDF
# -----------------------
    ...
    def create_pdf_bytes(data):
        ...
        himnos_tuples = data["himnos"]
        coros_tuples = data["coros"]
        coro_diezmo_num = data["num_diezmo"]
        coro_diezmo_tit = data["coro_diezmo"]
        coro_final_num = data["num_final"]
        coro_final_tit = data["coro_final"]
        ...
        pdf.cell(0, 6, clean_text_for_pdf(f"Fecha: {fecha_str}"), ln=True, align='C')
        pdf.ln(4)
    ...
    with col0:
        if st.button("📄 Generar PDF", use_container_width=True):
            # Tomamos los datos actuales del formulario
            now_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

            himnos_res = [
                (
                    str(st.session_state.get(f"him_num_{i}", "")).strip(),
                    himnos_df.loc[
                        himnos_df[him_num_col].astype(str).str.strip()
                        == str(st.session_state.get(f"him_num_{i}", "")).strip(),
                        him_tit_col
                    ].iloc[0]
                    if not himnos_df.loc[
                        himnos_df[him_num_col].astype(str).str.strip()
                        == str(st.session_state.get(f"him_num_{i}", "")).strip(),
                        him_tit_col
                    ].empty
                ...
                for i in range(st.session_state.config.get("himnos",0))
                if str(st.session_state.get(f"him_num_{i}", "")).strip() != ""
            ]

            coros_res = [
                (
                    str(st.session_state.get(f"coro_num_{i}", "")).strip(),
                    coros_df.loc[
                        coros_df[coro_num_col].astype(str).str.strip()
                        == str(st.session_state.get(f"coro_num_{i}", "")).strip(),
                        coro_tit_col
                    ].iloc[0]
                    if not coros_df.loc[
                ...
                for i in range(st.session_state.config.get("coros",0))
                if str(st.session_state.get(f"coro_num_{i}", "")).strip() != ""
            ]
            ...
            data = {
                ...
                "himnos": himnos_res,
                "coros": coros_res,
                "num_diezmo": num_diezmo,
                "coro_diezmo": coro_diezmo,
                "num_final": num_final,
                "coro_final": coro_final,
                "predicador": st.session_state.get("predicador",""),
                "notas": st.session_state.get("notas","")
            }

            pdf_bytes = create_pdf_bytes(data)
            st.session_state.last_data = data  # guardamos para vista previa

            st.download_button(
                "⬇️ Descargar PDF",
                data=pdf_bytes,
                file_name="guiador.pdf",
    ...
    # -----------------------
    # VISTA PREVIA EN PANTALLA (SIN PDF, SIN IFRAMES)
    # -----------------------
    if st.session_state.last_data is not None:
        d = st.session_state.last_data
        st.markdown("### 📄 Vista previa en pantalla (igual al contenido del PDF)")
        with st.container():
            ...
            with col_b:
                st.markdown('<p class="preview-title">GUIADOR IDMJI</p>', unsafe_allow_html=True)
                st.markdown(
                    f'<p class="preview-sub">Fecha: {d["fecha"]}</p>',
                    unsafe_allow_html=True
                )
            ...
            if d["himnos"]:
                st.markdown("**<< HIMNOS >>**")
                for num, tit in d["himnos"]:
                    if str(num).strip() != "" and str(tit).strip() != "":
                        st.write(f"{num}  {tit}")
                st.markdown("---")
            ...
            if d["coros"]:
                st.markdown("**<< COROS >>**")
                for num, tit in d["coros"]:
                    if str(num).strip() != "" and str(tit).strip() != "":
                        st.write(f"{num}  {tit}")
                st.markdown("---")

            # CORO DIEZMO
            if d["num_diezmo"] and d["coro_diezmo"]:
                st.markdown("**<< CORO DIEZMO >>**")
                st.write(f'{d["num_diezmo"]}  {d["coro_diezmo"]}')
                st.markdown("---")

            # CORO FINAL
            if d["num_final"] and d["coro_final"]:
                st.markdown("**<< CORO FINAL >>**")
                st.write(f'{d["num_final"]}  {d["coro_final"]}')
                st.markdown("---")

            # Predicador
            st.markdown("**PREDICADOR:**")
            st.write(d["predicador"] if d["predicador"] else "—")

            # Notas
            st.markdown("**NOTA:**")
            if d["notas"]:
                st.markdown(
                    f'<div style="background:#e6e6e6; padding:8px; border-radius:4px;">{d["notas"]}</div>',
                    unsafe_allow_html=True
                )
            else:
                st.markdown(
                    '<div style="background:#e6e6e6; padding:8px; border-radius:4px; color:#888;">(Sin notas)</div>',
                    unsafe_allow_html=True
                )
